...
namespace cartographer_ros
{
        ...
        map_builder_bridge_(node_options_, std::move(map_builder), tf_buffer)
  {
    ...
    service_servers_.push_back(node_handle_.advertiseService("switch_trajectory", &Node::HandleSwitchTrajectory, this));
    ...
    for (const auto &entry : map_builder_bridge_.GetLocalTrajectoryData())
    {
      ...
      if (trajectory_data.published_to_tracking != nullptr)
      {
        ...
        if (node_options_.publish_tracked_pose)
        {
          ...
          boost::array<double, 36> fake_covariance;
          std::fill(fake_covariance.begin(), fake_covariance.end(), 0.0);
          std::copy(fake_covariance.begin(), fake_covariance.end(), pose_msg.pose.covariance.begin());
    ...
    if (pure_localization_)
    {
      subscribers_[trajectory_id].push_back(
          {SubscribeWithHandler<geometry_msgs::PoseWithCovarianceStamped>(
               &Node::HandleInitPoseMessage, trajectory_id, "initialpose", &node_handle_, this),
           "initialpose"});
    }
  }
  ...
  bool Node::HandleSwitchTrajectory(::cartographer_ros_msgs::SwitchTrajectory::Request &request, ::cartographer_ros_msgs::SwitchTrajectory::Response &response)
  {
    ...
    Origin.position.x = 0.0, Origin.position.y = 0.0, Origin.position.z = 0.0;
    Origin.orientation.x = 0.0, Origin.orientation.y = 0.0, Origin.orientation.z = 0.0, Origin.orientation.w = 1.0;
    ...
    for (auto &element : pbstreams_)
    {
      if (directoryName == element.filename)
      {
        ...
        *initial_trajectory_pose.mutable_relative_pose() = cartographer::transform::ToProto(cartographer_ros::ToRigid3d(Origin));
        ...
        *trajectory_options.trajectory_builder_options.mutable_initial_trajectory_pose() = initial_trajectory_pose;
        response.trajectory_id = StartTrajectoryIDWithDefaultTopics(trajectory_options);
        return true;
      }
    }
  ...
  int Node::StartTrajectoryIDWithDefaultTopics(const TrajectoryOptions &options)
  {
    absl::MutexLock lock(&mutex_);
    CHECK(ValidateTrajectoryOptions(options));
    int trajectory_id = AddTrajectory(options);
    return trajectory_id;
  }
  ...
  void Node::HandleInitPoseMessage(const int trajectory_id, const std::string &sensor_id, const geometry_msgs::PoseWithCovarianceStamped::ConstPtr &msg)
  {
    FinishAllTrajectories();
    TrajectoryOptions trajectory_options;
    std::tie(std::ignore, trajectory_options) = LoadOptions(configuration_directory_, configuration_basename_);
